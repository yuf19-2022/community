...
import com.yuf19_2022.community.entity.Message;
import com.yuf19_2022.community.entity.Page;
import com.yuf19_2022.community.entity.User;
import com.yuf19_2022.community.service.MessageService;
...
@Controller
public class MessageController implements IDenoConstant {
    ...
    public String getLetterList(Model model, Page page) {
        ...
        //所有已建立私信会话的用户列表（用于在私信列表页面自动提送私信接收者）
        List<Message> messages = messageService.findConversations(user.getId(), 0, Integer.MAX_VALUE);
        List<User> users = new ArrayList<>();
        if (messages != null) {
            for (Message message : messages) {
                int targetId = user.getId() == message.getFromId() ? message.getToId() : message.getFromId();
                users.add(userService.findUserById(targetId));
            }
        }
